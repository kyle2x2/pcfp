...
(function() {
  ...
  function populateTaskList() {
    ...
    // Use paginated tasks instead of all tasks
    const tasksToRender = paginatedTasks.length > 0 ? paginatedTasks : tasks;
    
    taskGridBody.innerHTML = tasksToRender.map(task => {
  ...
  function applyPagination() {
    ...
    paginatedTasks = tasksToPaginate.slice(startIndex, endIndex);
  ...
  function updatePerformanceMetrics() {
    performanceMetrics.memoryUsage = new Blob([JSON.stringify(tasks)]).size;
  ...
  function deleteSelectedTasks() {
    ...
    if (confirm(`Are you sure you want to delete ${selectedCheckboxes.length} selected task(s)?`)) {
      ...
      // Show notification
      showNotification(`${selectedCheckboxes.length} task(s) deleted successfully`, 'success');
    }
  }
  ...
  function duplicateSelectedTasks() {
    ...
    const selectedIds = Array.from(selectedCheckboxes).map(cb => cb.dataset.taskId);
    const tasksToDuplicate = tasks.filter(task => selectedIds.includes(task.id));
    ...
    // Show notification
    showNotification(`${selectedCheckboxes.length} task(s) duplicated successfully`, 'success');
  }
  ...
  function exportSelectedTasks() {
    ...
    const selectedIds = Array.from(selectedCheckboxes).map(cb => cb.dataset.taskId);
    const tasksToExport = tasks.filter(task => selectedIds.includes(task.id));
    ...
    link.href = url;
    link.download = `schedule_tasks_${new Date().toISOString().split('T')[0]}.json`;
    link.click();
    
    URL.revokeObjectURL(url);
    showNotification(`${selectedCheckboxes.length} task(s) exported successfully`, 'success');
  }
