...
function setupEventListeners() {
    ...
    // Mass action buttons
    document.getElementById('btnDeleteSelected')?.addEventListener('click', deleteSelectedLogs);
...
function applyPagination() {
    ...
    totalPages = Math.ceil(logsToPaginate.length / itemsPerPage);
    ...
    paginatedLogs = logsToPaginate.slice(startIndex, endIndex);
...
function renderListView() {
    ...
    const logsToRender = paginatedLogs.length > 0 ? paginatedLogs : filteredLogs;
...
function duplicateLog(logId) {
    ...
    if (log) {
        ...
        applySearchAndFilter(); // This will update pagination and render
        saveLogs();
    }
}
...
function deleteSelectedLogs() {
    ...
    if (confirm(`Are you sure you want to delete ${selectedCheckboxes.length} selected log${selectedCheckboxes.length > 1 ? 's' : ''}?`)) {
        ...
        showNotification(`Deleted ${selectedCheckboxes.length} log${selectedCheckboxes.length > 1 ? 's' : ''}`, 'success');
    }
}
...
function duplicateSelectedLogs() {
    ...
    showNotification(`Duplicated ${selectedCheckboxes.length} log${selectedCheckboxes.length > 1 ? 's' : ''}`, 'success');
}
...
function exportSelectedLogs() {
    ...
    const logsToExport = Array.from(selectedCheckboxes).map(cb => window.dailyLogs.find(log => log.id === cb.dataset.logId));
    ...
    URL.revokeObjectURL(url);
    showNotification(`Exported ${selectedCheckboxes.length} log${selectedCheckboxes.length > 1 ? 's' : ''}`, 'success');
}
...
function updatePerformanceMetrics() {
    performanceMetrics.renderTime = performance.now() - (performanceMetrics.renderTime || 0);
    performanceMetrics.searchTime = performance.now() - (performanceMetrics.searchTime || 0);
    performanceMetrics.memoryUsage = new Blob([JSON.stringify(window.dailyLogs)]).size;
